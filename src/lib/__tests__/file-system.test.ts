...
test("reset clears all files and directories", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "content");
  fs.createDirectory("/src");
  fs.createFile("/src/index.ts", "export {}");
...
test("reset allows creating new files afterward", () => {
  const fs = new VirtualFileSystem();
...
test("replaceInFile handles regex special characters in search string", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "price is $10.00 and $10.00 again");

  const result = fs.replaceInFile("/test.txt", "$10.00", "$20.00");

  expect(result).toBe("Replaced 2 occurrence(s) of the string in /test.txt");
  expect(fs.readFile("/test.txt")).toBe("price is $20.00 and $20.00 again");
});

test("getAllFiles returns empty map for filesystem with only directories", () => {
  const fs = new VirtualFileSystem();
  fs.createDirectory("/src");
  fs.createDirectory("/src/components");
...
test("viewFile with view range beyond file length clamps to last line", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "line1\nline2\nline3");
...
test("viewFile with start line at 1 shows from beginning", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "a\nb\nc");

  const view = fs.viewFile("/test.txt", [1, 2]);
  expect(view).toBe("1\ta\n2\tb");
});

test("createDirectory returns null when parent does not exist", () => {
  const fs = new VirtualFileSystem();
...
test("deserialize clears previous state", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/old.txt", "old content");
...
test("deserializeFromNodes clears previous state", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/old.txt", "old");

  fs.deserializeFromNodes({
    "/": { type: "directory", name: "/", path: "/" },
    "/fresh.txt": { type: "file", name: "fresh.txt", path: "/fresh.txt", content: "fresh" },
  });

  expect(fs.exists("/old.txt")).toBe(false);
  expect(fs.readFile("/fresh.txt")).toBe("fresh");
});

test("insertInFile with multiline text", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "line1\nline3");

  const result = fs.insertInFile("/test.txt", 1, "line2a\nline2b");

  expect(result).toBe("Text inserted at line 1 in /test.txt");
  expect(fs.readFile("/test.txt")).toBe("line1\nline2a\nline2b\nline3");
});

test("replaceInFile with empty old string returns error", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "hello world");

  const result = fs.replaceInFile("/test.txt", "", "something");
